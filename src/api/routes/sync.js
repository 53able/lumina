...
import { SyncRequestSchema, SyncResponseSchema } from "../../shared/schemas";
...
export const syncRoute = createRoute({
    ...
    tags: ["sync"],
    summary: "arXiv論文の同期",
    description: "指定されたカテゴリでarXiv論文を取得し、Embeddingを生成します。クライアントは返されたデータをIndexedDBに保存します。",
    request: {
        body: {
            content: {
                "application/json": {
 ...
 */
const generatePaperEmbedding = async (paper, config) => {
...
syncApp.openapi(syncRoute, async (c) => {
    ...
    try {
        ...
        const papersWithEmbedding = await (async () => {
            try {
