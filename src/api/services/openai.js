 ...
 */
export const getOpenAIConfig = (c) => {
    const headerKey = c.req.header("X-OpenAI-API-Key");
    const envKey = process.env.OPENAI_API_KEY;
    const apiKey = headerKey ?? envKey;
    if (!apiKey) {
        throw new Error("OpenAI API key is not configured. Set OPENAI_API_KEY environment variable or pass X-OpenAI-API-Key header.");
    }
    return { apiKey };
};
/**
 ...
 */
export const createEmbedding = async (text, config) => {
 ...
 */
export const expandQuery = async (query, config) => {
    const provider = createProvider(config);
 ...
 * 4. 言語別の指示を分離して明確化
 */
export const generateSummary = async (abstract, language, config) => {
    const provider = createProvider(config);
 ...
 */
export const generateExplanation = async (abstract, language, config) => {
    const provider = createProvider(config);
    const langInstruction = language === "ja" ? EXPLANATION_INSTRUCTION_JA : EXPLANATION_INSTRUCTION_EN;
